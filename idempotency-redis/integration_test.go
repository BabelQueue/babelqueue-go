...
// liveStore connects to the URL in BABELQUEUE_TEST_REDIS, or skips the test cleanly
// when the env var is unset (the no-broker default in CI / local `go test ./...`).
// Set e.g.
//
//	BABELQUEUE_TEST_REDIS='redis://localhost:6379/0'
func liveStore(t *testing.T, opts ...redis.Option) *redis.Store {
	t.Helper()
	url := os.Getenv("BABELQUEUE_TEST_REDIS")
	if url == "" {
		t.Skip("set BABELQUEUE_TEST_REDIS to run the Redis integration tests")
	}
	store, err := redis.New(url, opts...)
	if err != nil {
		t.Skipf("bad redis url: %v", err)
	}
	// Probe reachability so an unreachable broker skips rather than fails.
	ctx, cancel := context.WithTimeout(context.Background(), 3*time.Second)
	defer cancel()
	if _, err := store.Seen(ctx, "ping"); err != nil {
		_ = store.Close()
...
// TestIntegration_ConcurrentClaimsSerialize proves the parking contract end to end:
// N goroutines race to Claim the SAME id against a live Redis; SET NX must let
// exactly ONE win. This is the in-flight serialization the in-memory store models,
// now durable across processes.
...
// TestIntegration_DuplicateRejected: the first Claim wins, a duplicate loses, and
// Seen reports true.
func TestIntegration_DuplicateRejected(t *testing.T) {
	...
	if seen, err := store.Seen(ctx, id); err != nil || !seen {
		t.Fatalf("Seen after claim = %v err=%v, want true", seen, err)
	}
}

// TestIntegration_TTLExpiryReclaims proves Redis honors PX: after a short TTL lapses
// the id is no longer Seen and may be re-claimed.
func TestIntegration_TTLExpiryReclaims(t *testing.T) {
	store := liveStore(t, redis.WithTTL(time.Second))
	ctx := context.Background()
	...
	time.Sleep(1500 * time.Millisecond) // let the PX TTL lapse

	if seen, err := store.Seen(ctx, id); err != nil || seen {
		t.Fatalf("Seen after expiry = %v err=%v, want false", seen, err)
	}
	if won, err := store.Claim(ctx, id); err != nil || !won {
